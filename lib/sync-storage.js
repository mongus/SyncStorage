    ...
    var instance = function(url, options) {
        ...
        var scheduleCacheUpdate = function(key) {
            ...
            pending[type] = setTimeout(function() {
                ...
                if (objects[type] instanceof List) {
                    if (objects[type].readOnly)
                        prefix = 'r:';
                    string = new SyncStorage.SyncStorageCsv().stringify(objects[type]);
                }
                else {
                    if (!(objects[type] instanceof Observable))
                        prefix = 'r:';
        ...
        var observer = function(changes) {
            changes.forEach(function(change) {
                ...
                // angular is adding properties :-P
                if (change.name.charAt(0) == '$')
                    return;
        ...
        var ListManager = {
            ...
            createClass: function(label) {
                ...
                eval.call(window, 'function ' + name + '() {};')
            }
        };
        ...
        var parse = function(key, string, readOnly) {
            ...
            new SyncStorage.SyncStorageCsv().parse(string, function(header) {
                ...
                ListManager.createClass(type, readOnly);
            }, function(row) {
                ...
                if (!readOnly)
                    Observable.mutate(object, key + ':' + id, observer);
        ...
        var updateObject = function(object, update) {
            ...
            if (update instanceof List)
                object.readOnly = update.readOnly;
        ...
        this.initTransport = function() {
            ...
            var handleIncomingData = function(header, string, readOnly) {
                ...
                if (!readOnly && update && !(update instanceof List) && !(update instanceof Observable)) {

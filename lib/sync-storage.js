...
(function() {
    ...
    var instance = function(url, options) {
        ...
        this.get = function(key, listener) {
            ...
            if (!containerExisted || !valueExisted || (key.id === null && !container._list)) {
                setTimeout(function() {
                    // ask the server to send the value
                    self.send({cmd: 'get', key: key.key});
                }, 0);
            }
        ...
        this.initTransport = function() {
            ...
            var handleIncomingData = function(header, string, readOnly) {
                ...
                var list = listeners[header.key];
                if (list) {
                    list.forEach(function(listener) {
                        listener(header.key, header.id !== null ? objects[header.type][header.id] : objects[header.type]);
                    });
                }
    ...
    window.SyncStorage = {
        ...
        connect: function() {
            ...
            if (!url)
                url = location.origin.replace(/:\d+$/, '')+':8081/sync-storage';
